...
//18.19 inserte los caracteres "******" en la mitad
 /*   getline(cin,primero);// '\n'
    ...
    getline(cin,primero);
    primero.insert(primero.size()/2,"******");
    cout<<"Cadena: "<<primero<<endl;

//18.21
    getline(cin,primero);
    Practica1::signToToken(primero);

//18.22Imprimir linea al revez
    getline(cin,primero);
    string::const_reverse_iterator i=primero.rbegin();
    while(i!=primero.rend()){
        cout<<*i;
        i++;
    }

//18.27    Escriba una versión de la rutina de ordenamiento por selección (fi gura 8.28) que ordene objetos string. Use la funciónswap en su solución
    string arr[]={"lo","esencial","es","invisible","a","los","ojos"};
    cout<<
    Practica1::ordenSeleccion(arr,7,Practica1::descendente);
    Practica1::printArr(arr,arr+7);
